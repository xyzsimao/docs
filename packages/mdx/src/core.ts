import type { CollectionItem, DocCollectionItem, LoadedConfig, MetaCollectionItem } from '@/config/build';
...
export function createCore(options: CoreOptions) {
  ...
  return {
    ...
    async transformMeta(options: TransformOptions<MetaCollectionItem>, data: unknown): Promise<unknown> {
      const ctx = {
    ...
    async transformVFile(options: TransformOptions<DocCollectionItem>, file: VFile): Promise<VFile> {
      const ctx = {
        ...this.getPluginContext(),
        ...options,
      };
